...
function ArrayMap(list, options) {
  ...
  defProp(self, 'maxSize', parseInt(options.maxSize, 10) || 0);
...
util._extend(ArrayMap.prototype, {
  ...
  forEach: wrapArrayMethod('forEach'),
  ...
  add: function(list) {
    if (arguments.length > 1 || isArray(list)) {
      ...
      for (var k in list) {
        if (list.hasOwnProperty(k)) this.set(k, list[k]);
      }
    }
  },
  ...
  each: function(fn, scope) {
    for (var i = -1, tuple, tuples = this._keys.slice(0), len = tuples.length; ++i < len; ) { tuple = tuples[i]
      // v, i, k, tuples
      if (fn.call(scope || tuple[1], tuple[1], i, tuple[0], tuples) === false) break;
    }
    return this
  },
  ...
  find: function(fn, scope) {
    var ret;
    this.each(function(v, i, k, tuples) {
      if (fn.call(scope || v, v, k, tuples)) {
        ret = v;
        return false;
      }
    });
...
// Trim the queue down to the appropriate size, removing
// items from the beginning of the internal array.
function trimHead(self) {
  var length = self.length, limit = self.maxSize;
